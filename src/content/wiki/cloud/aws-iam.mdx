...
aliases: ["IAM", "AWS IAM", "IAM Role", "IAM Policy", "STS", "AssumeRole", "Identity Center", "SSO", "Permission Boundary", "권한 경계"]
tags: [aws, iam, security, auth, cloud, governance]
category: cloud
updated: 2026-07-30
references:
  - title: "AWS IAM"
...
| 종류 | 부착 대상 | 권한 부여? | 의미 |
|:---|:---|:---|:---|
| **AWS Managed** | User/Group/Role | O | AWS 가 관리 (`AmazonS3ReadOnlyAccess`) |
| **Customer Managed** | User/Group/Role | O | 사용자 정의, 재사용 가능 |
| **Inline** | User/Group/Role | O | 직접 임베드 (재사용 X) |
| **Resource-based Policy** | S3/SQS/KMS/Lambda 등 | O | 리소스에 붙음, 교차 계정 허용 |
| **Permission Boundary** | User/Role | **X (상한만)** | Role 이 가질 수 있는 최대 |
| **Service Control Policy (SCP)** | Org / OU / Account | **X (상한만)** | 계정 내 principal 최대 권한 |
| **Resource Control Policy (RCP, 2024+)** | Org / OU / Account | **X (상한만)** | 계정 내 resource 최대 권한 |
| **Session Policy** | AssumeRole 세션 | **X (상한만)** | 임시 세션에 상한 |
...
자세히: [[aws-organizations|AWS Organizations]] SCP/RCP.
...
- [[aws-organizations|AWS Organizations]] - SCP/RCP, 다중 계정
- [[aws-sts-assume-role|STS AssumeRole]] - 임시 자격 증명
- [[aws-secrets-manager|Secrets Manager]] - 비밀 관리
- [[aws-systems-manager|Systems Manager]] - Parameter Store, Session Manager
- [[aws-cloudtrail|CloudTrail]] - IAM 감사 로그
- [[aws-lake-formation|Lake Formation]] - 데이터 레이크 세분화 권한
- [[aws-kms|KMS]] - 암호화 키 정책
- [[aws-eks|EKS]] (IRSA)
- [[OAuth2]] (비교)
- [[k8s-rbac]] (대조)
