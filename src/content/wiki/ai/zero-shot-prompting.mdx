...
| 한계 상황 | 원인 | 대안 |
|:---|:---|:---|
| 사내 고유 분류 체계 | 지시문으로 표현 어려움 | few-shot 예시 추가 |
| 10개 이상 라벨 분류 | 라벨 집합이 너무 큼 | few-shot 또는 파인튜닝 |
| 엄격한 구조 출력 | 형식 오류율 높음 | [[function-calling|Function Calling]] / JSON mode |
| 도메인 특화 어휘 | 사전학습에 희소 | few-shot + [[llm-rag|RAG]] |
| 수치 추론 및 계산 | 직접 계산 실수 | zero-shot CoT 또는 code interpreter |
