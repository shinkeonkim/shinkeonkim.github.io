...
```mermaid
flowchart LR
    subgraph "원형 버킷 배열 (크기 W+1)"
        B0["bucket[0]<br/>cur=0"]
        B1["bucket[1]"]
        B2["bucket[2]"]
        BW["bucket[W]"]
    end
...
dist[v] 값을 `W+1` 로 나눈 나머지가 버킷 인덱스. 최소 dist 가 `d` 이면 유효 범위는 `d ~ d+W` 이므로 크기 `W+1` 원형 배열이면 충분.

## Dijkstra vs Dial 선택 기준

```mermaid
flowchart TD
    Q["가중치 특성?"] -->|"정수 + W 작음"| D["Dial O(V+E+W)"]
    Q -->|"실수 / W 큼"| H["Heap Dijkstra O((V+E) log V)"]
    Q -->|"음수 포함"| BF["Bellman-Ford O(VE)"]
    Q -->|"가중치 0 또는 1"| BFS0["0-1 BFS O(V+E)"]
    D -->|"W > sqrt(E)"| H
```
...
|:---|:---|:---|
| BFS | O(V + E) | 무가중 |
| 0-1 BFS | O(V + E) | 가중치 0 또는 1 |
| Dial | O(V + E + W) | 정수 가중치, W 작음 |
| Heap Dijkstra | O((V+E) log V) | 비음수 실수 |
| Bellman-Ford | O(VE) | 음수 허용 |

**선택 기준**: W <= sqrt(E) 이면 Dial 이 유리 (버킷 오버헤드 < log V). W 가 크면 일반 heap.

## 적용 가능한 그래프 유형

| 도메인 | 그래프 | W 범위 | 이점 |
|:---|:---|:---|:---|
| 게임 타일맵 | 격자 이동 비용 1-5 | 1-5 | O(V+E+5) vs O(E log V) |
| 교통망 | 도시 간 거리 (단위 km) | 0-200 | 실측 데이터 의존 |
| 네트워크 라우팅 | hop count | 0-255 | 라우터 최적화 |
| 이미지 처리 | 픽셀 거리 | 0-255 | 대규모 픽셀 처리 |
