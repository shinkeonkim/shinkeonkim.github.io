...
Barrett Reduction 의 단계별 연산:

```mermaid
flowchart TD
    Pre["사전 계산: M = 2^k / m"] --> Ready["Barrett 준비"]
    Ready --> |"x 입력"| Mul["q = x * M >> k"]
    Mul --> Corr["r = x - q * m"]
    Corr --> |"r >= m"| Fix["r -= m"]
    Corr --> |"r < m"| Done["결과 반환"]
    Fix --> Done
...
전체 경로: 128비트 곱셈 1 + 시프트 1 + 뺄셈 1 + 조건 뺄셈 최대 1. 총 ~5 사이클.

## Python 구현 (이해 목적)

PS 환경에서 Python 은 정수 연산이 느리지만, Barrett 의 아이디어를 Python 에서도 확인해볼 수 있다:

```python
class Barrett:
    def __init__(self, m: int):
        self.m = m
        self.k = m.bit_length() * 2   # k = 2 * bit_length(m)
        self.M = (1 << self.k) // m   # 사전 계산

    def reduce(self, x: int) -> int:
        q = (x * self.M) >> self.k    # 몫 근사
        r = x - q * self.m
        if r >= self.m:
            r -= self.m
        return r
...
assert bt.reduce(10**15) == 10**15 % 998244353

# 실전에서는 C++ Barrett 구현 사용 권장
# PyPy + ctypes 를 쓰더라도 C++ 보다 느림
```

## 컴파일러 자동 최적화와의 관계

```cpp
// Case 1: 컴파일 시 mod 값 알 때
// 컴파일러가 자동으로 magic number 곱셈으로 최적화 (Barrett 과 유사)
x % 998244353;  // -> 컴파일러가 div 대신 곱셈+시프트로 변환

// Case 2: 런타임 mod (동적)
// 컴파일러가 최적화 불가 -> div 명령 발생 -> Barrett 수동 구현 필요
int m;
cin >> m;
x % m;  // -> div 명령 (느림)
```

| 조건 | 컴파일러 최적화 | Barrett 필요 여부 |
|:---|:---|:---|
| mod 값이 컴파일 시 상수 | 자동 magic number | 불필요 |
| mod 값이 런타임 결정 | div 명령 그대로 | 필요 |
| mod 값이 루프 안 변경 | div 명령 그대로 | 필요 |

> [!TIP]
> NTT 처럼 mod 가 런타임에 고정되는 경우 (사용자 입력 N) 에 Barrett 이 가장 큰 효과. 고정 mod (998244353 등) 는 `constexpr` 로 선언하면 컴파일러가 대신 최적화.
