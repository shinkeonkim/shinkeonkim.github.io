...
```mermaid
flowchart TD
    A["배열 입력"] --> B["buildMaxHeap<br/>O(n)"]
    B --> C["max-heap 완성"]
    C --> D["루트(최댓값)와<br/>마지막 원소 swap"]
    D --> E["heap 크기 1 감소"]
    E --> F["heapifyDown<br/>O(log n)"]
    F --> G{heap 크기 > 1}
    G -->|"Yes"| D
    G -->|"No"| H["정렬 완료<br/>O(n log n)"]
```

## Priority Queue 변형: D-ary Heap

바이너리 힙 (2-ary) 외에 **D-ary Heap** 으로 일반화 가능:

| 속성 | Binary Heap | D-ary Heap |
|:---|:---|:---|
| **자식 수** | 2 | D |
| **insert** | O(log₂ n) | O(log_D n) |
| **delete-min** | O(log₂ n) | O(D log_D n) |
| **캐시 친화성** | △ | ○ (D 크면 트리 낮아짐) |
| **권장 D** | - | 4 또는 8 |

D = 4 인 4-ary heap 은 **insert 가 잦고 delete-min 이 드문** 경우 (예: Dijkstra) 에서 바이너리 힙보다 실측 빠름. 트리 높이가 log_D(n) 으로 낮아 캐시 지역성이 개선된다.

```python
# D-ary Heap 헬퍼 (D=4 예시)
D = 4

def parent(i):      return (i - 1) // D
def children(i):    return range(i * D + 1, i * D + D + 1)
...
| 상황 | 사용 여부 | 대안 |
|:---|:---|:---|
| **전체 정렬 (in-memory)** | 비권장 | Timsort / Quicksort |
| **최악 시간 보장 필수** | 권장 | Introsort 폴백으로 자동 사용 |
| **Top-K 추출** | 강력 권장 | O(n log K) |
| **Priority Queue** | 기본 | 직접 구현 or 언어 내장 |
| **K-way merge** | 권장 | 외부 정렬 |
| **실시간 스케줄러** | 권장 | 운영체제 내부 |
| **안정 정렬 필요** | 비권장 | Timsort / Merge Sort |
