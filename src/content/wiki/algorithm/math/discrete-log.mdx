...
references:
  - id: solved-ac
    note: "tag: discrete_log"
  - id: cp-algorithms
    note: "algebra/discrete-log"
---
...
x 를 i·m_sqrt + j 로 분해해 양방향에서 수렴하는 구조를 요약하면 다음과 같다.

```mermaid
flowchart TD
    Input["입력: a, b, m"] --> CalcN["m_sqrt 계산"]
    CalcN --> BabyStep["Baby Step: table 구축"]
    BabyStep --> GiantStep["Giant Step 시작"]
    GiantStep --> Check{명중}
    Check -->|"명중"| ReturnX["x 반환"]
    Check -->|"계속"| Next["cur 갱신, i++"]
    Next --> GiantStep
    Check -->|"범위 초과"| NotFound["-1 반환"]
```

| 단계 | 수행 | 반복 횟수 |
|:---|:---|:---|
| Baby Step | `a^j mod m` 을 hash map 에 채운다 | O(√m) |
| Giant Step | `b*(a^(-m_sqrt))^i mod m` 이 map 에 있는지 확인 | O(√m) |
| 전체 | 시간 + 공간 모두 O(√m) | |
...
이산 로그 문제는 알고리즘 연습을 넘어 **현대 암호학의 어려운 문제** 로 쓰인다.

### Diffie-Hellman 키 교환

```text
g^a mod p = A   (Alice: a 비밀, A 공개)
g^b mod p = B   (Bob: b 비밀, B 공개)
A^b mod p = g^(a*b) = B^a mod p   (공유 비밀)
```

공격자는 `g^a mod p = A` 에서 `a` 를 역산해야 한다. p 가 수백 비트 소수면 BSGS 로도 불가능 (√p 연산이 천문학적).

### ElGamal 암호

ElGamal 체계도 이산 로그의 어려움을 안전성 기반으로 삼는다. 암호화에 랜덤 `k` 를 사용해 같은 메시지도 매번 다른 암호문이 생성된다.

### ECDLP (타원 곡선 이산 로그)

유한 체 대신 타원 곡선 군 위에서의 이산 로그. 같은 보안 수준에 훨씬 짧은 키 사이즈. TLS, Bitcoin, JWT ES256 등이 사용.

### 대회 적용 패턴

- **단독 출제**: `a^x ≡ b (mod p)`, p ≤ 10^9 범위에서 BSGS 직접 적용
- **복합 출제**: p-1 의 소인수분해가 가능한 경우 Pohlig-Hellman 후 BSGS
- **암호 공격 문제**: 작은 p 로 DLP 를 풀어 비밀키 복원

> [!IMPORTANT]
> BSGS 는 a 와 m 이 서로소여야 `a^(-m_sqrt) mod m` 이 존재한다. 서로소가 아니면 확장 BSGS (Extended BSGS) 로 전처리 후 적용해야 한다.
