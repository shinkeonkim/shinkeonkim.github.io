...
세 케이스가 차례로 적용되면서 `(a, m)` 쌍이 유클리드 호제법과 동일한 구조로 수렴한다. 각 재귀 호출마다 인자가 `(n, m, a, b)` 에서 `(y_max, a, m, new_b)` 로 전환되어 `m` 과 `a` 의 역할이 뒤바뀐다.

```mermaid
flowchart TD
    A["floor_sum(n, m, a, b)"]
    B{"a >= m ?"}
    C["Case 1: 몫 분리, a = a mod m"]
    D{"b >= m ?"}
    E["Case 2: 상수 분리, b = b mod m"]
    F["y_max = floor((a*(n-1)+b) / m)"]
    G{"y_max = 0 ?"}
    H["return ans"]
    I["Case 3: 기하 변환, 재귀 호출"]
    A --> B
    B -->|"Yes"| C --> B
    B -->|"No"| D
    D -->|"Yes"| E --> D
...
재귀 호출마다 `(a, m)` 이 `(m mod a, a)` 방향으로 감소한다. 유클리드 호제법과 동일한 수렴 속도이므로 `m, a ≤ 10^18` 기준 최대 약 87 회의 재귀로 종료.

## 격자점 해석

`floor_sum(n, m, a, b)` 는 직선 `y = (a·x + b) / m` 아래, x 축 위의 정수 격자점 수와 동치다:

```text
S = { (x, y) : 정수 x, y, 0 <= x < n, 1 <= y <= floor((a*x + b) / m) }
|S| = floor_sum(n, m, a, b)
```

| 케이스 | 기하 의미 |
|:---|:---|
| `a >= m` | 직선 기울기를 1 미만으로 줄이고 정수 부분을 ans 에 누적 |
| `b >= m` | y 절편의 정수 부분을 먼저 분리 |
| 기하 변환 | x-y 대칭: 직사각형 내 격자점 = 직선 아래 점 + 직선 위 점 |

기하 변환의 핵심: 직사각형 `[0, n) x [0, y_max)` 에서 격자점을 y 방향으로 세면 새 `floor_sum` 의 인자가 `(y_max, a, m, new_b)` 로 자연스럽게 유도된다. 이 변환에서 `m` 과 `a` 가 뒤바뀌는 것이 알고리즘 수렴의 원천.

> [!TIP]
> 네 번째 인자 `(a - 1) - (b % a)` 는 암기보다 격자점 대칭 변환을 한 번 직접 추적하는 것이 좋다. AtCoder Library 의 `floor_sum.hpp` 주석에도 이 해석이 기술되어 있다.
