...
각 스텝에서:
1. `dq.front()` 가 `i - K` 보다 작으면 만료 제거
2. `dp[i]` 는 `dp[dq.front()] + a[i]` 로 계산
3. `dq.back()` 이 dp 값이 새 값보다 작으면 제거 (단조 감소 유지)
4. `i` 를 `dq.back()` 에 추가
...
1. `j in [i-K, i-1]` 형태로 윈도우가 명확히 고정: 덱 DP
2. `opt(i,j)` 가 단조 (monotone minima 성질): Divide and Conquer DP
3. 위 둘 다 아닌 경우: 세그먼트 트리 O(N log N)
