...
일반 TLS 는 서버만 인증서를 제시한다. mTLS 는 **클라이언트도 인증서를 제시**해 양방향 인증을 수행한다.
...
| 방식 | TLS 버전 | 서버 상태 | 설명 |
|:---|:---|:---|:---|
| Session ID | 1.2 | 서버 저장 | 서버가 ID 반환, 클라이언트가 다음 연결 때 제시 |
| Session Ticket | 1.2 / 1.3 | 클라이언트 저장 | 서버가 암호화된 상태를 티켓으로 전달 |
| PSK / 0-RTT | 1.3 | 없음 | 이전 세션 키로 첫 패킷에 데이터 포함 |
...
- [[network-dns|DNS]] - 인증서 발급 시 도메인 소유 증명
