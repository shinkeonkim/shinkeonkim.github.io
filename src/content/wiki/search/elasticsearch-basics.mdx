---
title: "[Search] ElasticSearch 기본 원리: Inverted Index, Segment, Lucene"
aliases:
  - "ES 기본 원리"
  - "inverted index"
  - "역색인"
  - "Lucene segment"
  - "near real-time"
  - "refresh interval"
  - "translog"
  - "Lucene"
tags: [elasticsearch, lucene, inverted-index, segment, internals]
category: search
updated: 2026-06-25
references:
  - title: "Elastic: Inverted index"
    url: "https://www.elastic.co/guide/en/elasticsearch/reference/current/_basic_concepts.html"
  - title: "Apache Lucene"
    url: "https://lucene.apache.org/"
---

## 정의

ES 의 *모든 동작* 은 *Lucene 의 inverted index* 위에 얹혀있다. *문서 단위 저장* + *term 단위 검색* 의 분리.

## Inverted Index (역색인)

```anim:trie
{}
```

> 위 trie 의 *prefix tree 구조* 가 *term dictionary* 의 직관. Lucene 은 *FST (Finite State Transducer)* 라는 *압축된 trie* 사용.

### 일반 정방향 인덱스 vs 역색인

```mermaid
flowchart LR
    subgraph Forward["Forward Index (DB)"]
        D1["doc 1: 'rabbit hat'"]
        D2["doc 2: 'red rabbit'"]
        D3["doc 3: 'big hat'"]
    end
    subgraph Inverted["Inverted Index (Lucene)"]
        T1["'rabbit' → [1, 2]"]
        T2["'hat' → [1, 3]"]
        T3["'red' → [2]"]
        T4["'big' → [3]"]
    end
```

| 정방향 | 역색인 |
|---|---|
| doc → terms | *term → docs* |
| RDB 의 row | 검색에 최적 |
| `LIKE '%rabbit%'` = O(N) | *term 으로 즉시 lookup* |

## 검색 흐름

```mermaid
sequenceDiagram
    autonumber
    Client->>ES: GET /idx/_search { "match": { "title": "rabbit hat" } }
    ES->>Analyzer: 쿼리 분석 (tokenize)
    Analyzer-->>ES: tokens: ["rabbit", "hat"]
    ES->>Inv: term lookup
    Inv-->>ES: postings: rabbit=[1,2], hat=[1,3]
    ES->>ES: intersection / union (BM25 score 계산)
    ES->>ES: 정렬 (score desc)
    ES-->>Client: top hits
```

## Segment: Lucene 의 단위

```mermaid
flowchart TB
    Shard[Shard]
    Shard --> Seg1["Segment 1<br/>(불변)"]
    Shard --> Seg2["Segment 2<br/>(불변)"]
    Shard --> Seg3["Segment 3<br/>(불변)"]
    Shard --> Seg4["Segment N<br/>(최신)"]
```

| 속성 | 의미 |
|---|---|
| **Immutable** | segment 는 *생성 후 절대 변경 안 됨* |
| **Append-only** | 새 문서 = *새 segment* |
| **Merge** | 여러 segment 를 *주기적으로 큰 하나로* (background) |
| **Delete** | 삭제 = *별도 `.del` 파일에 표시* (실제 제거는 merge 시) |

> [!IMPORTANT]
> Segment 의 *불변성* 이 ES 의 *concurrent search + write 가능* 의 토대. 동시 읽기에 lock 없음.

## Near Real-Time: refresh

```mermaid
sequenceDiagram
    autonumber
    participant App
    participant Buf as In-memory Buffer
    participant FS as FS Cache (segment)
    participant Disk

    App->>Buf: index doc (즉시 검색 안 됨)
    Note over Buf: refresh interval (1초 기본)
    Buf->>FS: refresh → 새 segment (검색 가능!)
    Note over FS: 메모리 cache 에 있을 뿐 디스크 아직
    Note over FS: flush 주기
    FS->>Disk: fsync (durable)
```

| 동작 | 주기 | 의미 |
|---|---|---|
| **Buffer → Refresh** | 1초 (`refresh_interval`) | 검색 가능 (still in memory) |
| **Refresh → Flush** | `index.translog.flush_threshold_size` | 디스크 fsync |
| **Merge** | 백그라운드 | 작은 segment 들 → 큰 segment |

> [!TIP]
> *대량 인덱싱* 시 `refresh_interval: -1` 로 끄면 *수배 빠름*. 인덱싱 후 명시적 `_refresh`.

## Translog (WAL)

```mermaid
flowchart LR
    Write[Index API] --> Buf[Buffer]
    Write --> TL["Translog<br/>(WAL)"]
    TL -->|fsync 주기| Disk
```

*Refresh* 와 무관하게 *write 가 안전*. 자세한 건 [[wal-write-ahead-log]].

| 모드 | 동작 |
|---|---|
| `request` (기본) | 매 요청 fsync (느림, 안전) |
| `async` | 주기적 fsync (5s 기본) |

## Document 와 ID

```json
PUT /products/_doc/sku-1234
{
  "name": "Mechanical Keyboard",
  "price": 129.99,
  "tags": ["accessory", "input"]
}
```

| 필드 | 의미 |
|---|---|
| `_index` | 인덱스 이름 |
| `_id` | 문서 ID (없으면 자동 생성) |
| `_source` | 원본 JSON (저장 + 반환용) |
| `_version` | 낙관적 동시성 제어 |
| `_seq_no`, `_primary_term` | versioning 의 개선판 |

## Routing

```
shard = hash(routing) % number_of_primary_shards
```

- 기본 `routing = _id`.
- *같은 routing → 같은 shard*.
- *멀티 테넌트* 에서 `routing = tenant_id` 로 *데이터 locality*.

## Analyzer 파이프라인

문서를 인덱싱하거나 검색어를 처리할 때, ES 는 *Analyzer* 를 통해 텍스트를 *token* 의 스트림으로 변환한다.

```mermaid
flowchart LR
    Raw["원문 텍스트<br/>e.g. 'Elastic Search 2024!'"] --> CT[Character Filter]
    CT --> TOK[Tokenizer]
    TOK --> TF[Token Filter]
    TF --> Tokens["tokens<br/>[elastic, search, 2024]"]
```

| 단계 | 역할 | 예 |
|---|---|---|
| Character Filter | 문자 수준 변환 | HTML strip, 특수문자 치환 |
| Tokenizer | 텍스트를 token 으로 분리 | `standard`, `whitespace`, `nori` (한국어) |
| Token Filter | token 변환/추가/삭제 | lowercase, stopwords, synonym, stemmer |

```json
PUT /my-index
{
  "settings": {
    "analysis": {
      "analyzer": {
        "my_analyzer": {
          "type": "custom",
          "tokenizer": "standard",
          "filter": ["lowercase", "stop"]
        }
      }
    }
  }
}
```

한국어 분석은 [[elasticsearch-korean-indexing]] 참고.

## Mapping: 스키마 정의

RDB 의 DDL 에 해당. dynamic mapping 으로 자동 추론되지만 프로덕션에서는 명시적 mapping 을 쓴다.

```json
PUT /products/_mapping
{
  "properties": {
    "name":  { "type": "text",    "analyzer": "standard" },
    "price": { "type": "double" },
    "tags":  { "type": "keyword" },
    "created_at": { "type": "date", "format": "yyyy-MM-dd" }
  }
}
```

| 필드 타입 | 설명 |
|---|---|
| `text` | 분석 후 역색인. 전문 검색용 |
| `keyword` | 분석 없이 그대로 인덱싱. 정렬, aggregation, 정확 매치 |
| `date` | 날짜/시간. ISO 8601 또는 custom format |
| `double` / `long` | 숫자. range 쿼리, 집계 |
| `nested` | 배열 내 객체를 독립 문서처럼 처리 |
| `dense_vector` | 벡터 검색 (kNN) 용 |

자세히는 [[elasticsearch-mapping]].

## 흔한 함정

> [!WARNING]
> 1. **`text` vs `keyword` 혼동** = `text` 는 집계/정렬 불가. `.keyword` sub-field 사용.
> 2. **Dynamic mapping 무제한** = 스키마 폭발 (`_doc_count_error`). mapping explosion 방지 설정 필요.
> 3. **Shard 과다** = 1000개 이상 shard 는 오히려 느림. shard per node 기준 고려.
> 4. **refresh 빈번** = `_refresh` 를 수동으로 자주 부르면 segment 급증 → merge pressure.
> 5. **update = delete + reindex** = ES 의 update 는 내부적으로 기존 doc 를 soft delete 후 새 doc 생성. 잦은 update 시 segment 비대해짐.

## 자세한 영역

- *인덱싱 흐름 + mapping*: [[elasticsearch-indexing]]
- *쿼리 종류*: [[elasticsearch-query]]
- *cluster, shard*: [[elasticsearch-infrastructure]]
- *BM25*: [[elasticsearch-relevance-scoring]]
- *한국어 분석*: [[elasticsearch-korean-indexing]]
- *aggregation*: [[elasticsearch-aggregations]]

## 관련 위키

- [[elasticsearch]] (개요)
- [[btree-indexing]] (RDB 인덱스 비교)
- [[database-internals/redis-vector-search|Redis Vector Search]] (대안)
- [[trie]] (FST 의 토대)
