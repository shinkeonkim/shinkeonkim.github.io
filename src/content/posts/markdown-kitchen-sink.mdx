...
`quiz` 코드 fence 로 4 가지 유형의 문제를 만들 수 있습니다.

```quiz
{
  "title": "이 데모 페이지 확인 문제",
  "items": [
    {
      "type": "multiple-choice",
      "question": "이 블로그가 사용하는 정적 사이트 생성기는?",
      "options": ["Astro", "Next.js", "Hugo", "Jekyll"],
      "correct": 0,
      "explanation": "Astro + MDX + GitHub Pages 로 배포됩니다."
    },
    {
      "type": "multiple-select",
      "question": "본 페이지에서 다룬 marks 다운 확장 문법은? (여러 개)",
      "options": ["위키링크", "수식(KaTeX)", "Mermaid 다이어그램", "React 컴포넌트"],
      "correct": [0, 1, 2, 3],
      "explanation": "모두 지원됩니다."
    },
    {
      "type": "true-false",
      "question": "em-dash (\u2014) 는 이 블로그에서 허용되지 않는다.",
      "correct": true,
      "explanation": "prebuild 단계에서 U+2014 사용 시 빌드가 실패합니다."
    },
    {
      "type": "short-answer",
      "question": "이 블로그가 검색 인덱스로 사용하는 도구 이름은?",
      "correct": "Pagefind",
      "explanation": "정적 사이트에 최적화된 클라이언트 사이드 검색 라이브러리입니다."
    }
  ]
}
```
